...
class CVAETrainer(Trainer):
  ...
  def sample_digits(self):
    ...
    for i in range(50): 
      ...
      latent_sample = torch.randn(1, self.model.latent_dim).to(self.device)

      with torch.no_grad(): 
        y = torch.tensor([col]).to(self.device)
        x_sample = self.model.decoder(latent_sample, y)
        x_sample = x_sample.reshape(28, 28).cpu().numpy()

      # Plot the image
      axes[row, col].imshow(x_sample, cmap='gray')
  ...
  def reconstruct(self): 
    ...
    x = x.squeeze(1)[:25]
    y = y[:25]
    xhat, _, _ = self.model(x.view(-1, 28 * 28).to(self.device), y.to(self.device))
  ...
  def interpolate(self):
    ...
    def get_centroid(x, label):
      ...
      labels = torch.full((x.size(0),), label, dtype=torch.long, device=self.device)
      Z, _ = self.model.encoder(data, labels)
    ...
    def get_a2b(a_label: int, b_label: int):
      ...
      z_a = get_centroid(x_a, a_label)
      z_b = get_centroid(x_b, b_label)
      z_a2b = z_b - z_a
      return z_a2b
    ...
    def interpolate_(a_label = 0):
      ...
      labels = torch.full((x_a.size(0),), a_label, dtype=torch.long, device=self.device)
      z_a, _ = self.model.encode(data, labels)
      z_in = z_a[0]
      ...
      for z_a2b in z_a2b_all:
          for alpha in np.arange(0, 2, 0.2):
              z = z_in + alpha*z_a2b
              # Decode using the source label 'a_label'
              y_in = torch.tensor([a_label], device=self.device).reshape(1)
              x_vae = self.model.decode(z.unsqueeze(0), y_in).detach()
              x_interpolated.append(x_vae.squeeze(0))
  ...
  def visualize_latent_space(self):
    ...
    with torch.no_grad():
      for batch in self.val_dl:
        ...
        mu, _ = self.model.encode(x, y_dev)
